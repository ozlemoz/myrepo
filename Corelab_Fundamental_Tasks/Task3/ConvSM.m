function [SER_opt_conv, SER_subopt_conv] = ConvSM()
NumBit = 10^6;
...
SER_opt_conv(1,length(SNRdB)) = zeros;
SER4(1,length(SNRdB)) = zeros;
SER5(1,length(SNRdB)) = zeros;
SER_subopt_conv(1,length(SNRdB)) = zeros;
...
%     SER2(xx) = symbol_errors2(xx)/Num;
%     symbol_errors2=zeros(length(Es_N0_dB),1);
%     SER3(xx) = symbol_errors3(xx)/Num;
%     symbol_errors3=zeros(length(Es_N0_dB),1);
    SER_opt_conv(k) = symbol_errors3(k)/(3*NumBit);
    symbol_errors3=zeros(length(SNRdB),1);
    SER_subopt_conv(k) = symbol_errors6(k)/(3*NumBit);
    symbol_errors6=zeros(length(SNRdB),1);
end
end