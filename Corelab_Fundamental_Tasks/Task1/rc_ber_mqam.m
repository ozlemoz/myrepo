function [BERsim_mqam, BERthe_mqam] = rc_ber_mqam(NumBit,M,SNRdB)
...
for k = 1:length(SNRdB)
    ...
    h = 1/sqrt(2)*(randn(1,NumBit) + 1i*randn(1,NumBit));
    y = s.*h + 10^(-Es_N0_dB(k)/20)*n; % additive white gaussian noise
    y = y./h;
...
%     if(M_qam == 16)
%         BERthe_mqam(k) = 3/8 * ( 1 - sqrt(2/5*gamma_c/p./(1+2/5*gamma_c/p)) );
%     end
%     
%     if(M_qam == 64)
%         BERthe_mqam(k) = 7/24 * ( 1 - sqrt(1/7*gamma_c/p./(1+1/7*gamma_c/p)) );        
%     end

end 

BERthe_mqam = berfading(SNRdB,'qam',M_qam,1);
